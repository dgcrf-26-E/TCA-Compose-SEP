...
class Oficina(models.Model):
    id = models.AutoField(primary_key=True)
    estado = models.ForeignKey(Estados, on_delete=models.CASCADE, related_name='oficinas')
    sede = models.CharField(max_length=100)
    abrev = models.CharField(max_length=20)

    def __str__(self):
        return f"{self.id}, {self.sede} ({self.abrev})"
...
class Registro(models.Model):
    ...
    claveAcuerdo = models.CharField(default="Clave de Acuerdo", max_length=30)
    fecha_inicio = models.DateField()
    fecha_termino = models.DateField()
    rubro = models.ManyToManyField(Rubro, related_name='registroR')
    area = models.ManyToManyField(Oficina, related_name='registroA')
...
class Acciones(models.Model):
    ...
    area2 = models.ManyToManyField(Oficina, related_name='accionA2')
    ...
    def __str__(self):
        return "Acción: {accion}, Registros: {datosRegistro}, Área 2: {area2}, Descripción: {descripcionI}".format(
            ...
            area2= ', '.join([area.abrev for area in self.area2.all()]),
...
class UsuarioP(models.Model):
    ...
    oficina = models.ForeignKey(Oficina, on_delete=models.SET_NULL, null=True, blank=True, related_name='usuarios')
    ...
    def __str__(self):
        return  " {id}, {nickname}, {state}, {type}".format(id = self.idUser, nickname = self.nickname, state = self.oficina, type = self.tipo)
